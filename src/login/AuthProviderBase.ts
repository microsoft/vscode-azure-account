...
export abstract class AuthProviderBase<TLoginResult> {
	...
	public abstract loginSilent(environment: Environment, tenantId: string, migrateToken?: boolean, resource?: string): Promise<TLoginResult>;
	...
	public static getAccessTokenFromLoginResult(loginResult: AbstractLoginResult): string {
		if (Array.isArray(loginResult)) {
			return loginResult[0].accessToken;
		} else {
