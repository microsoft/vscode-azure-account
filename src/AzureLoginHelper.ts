...
import { azureCustomCloud, azurePPE, cacheKey, clientId, commonTenantId, customCloudArmUrlKey, displayName, staticEnvironments } from './constants';
...
export class AzureLoginHelper {
	...
	private async loginToCloud(): Promise<void> {
		...
		const selected: QuickPickItem & { environment: Environment } | undefined = await window.showQuickPick<{ label: string, description?: string, environment: Environment }>(getEnvironments(true /* includePartial */)
