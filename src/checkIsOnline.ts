...
export async function checkIsOnline(environment: Environment, interval: number, token = new CancellationTokenSource().token): Promise<void> {
	let checkIsOnlineTask: Promise<boolean> = checkIsOnlineHelper(environment);
	...
	while (!token.isCancellationRequested && !await Promise.race([checkIsOnlineTask, delayTask])) {
		await delayTask;
		checkIsOnlineTask = asyncOr(checkIsOnlineTask, checkIsOnlineHelper(environment));
		delayTask = delay(interval, false);
	}
}

async function checkIsOnlineHelper(environment: Environment): Promise<boolean> {
	try {
